  ...
  try {
    ...
    const [placeRes, reviewsRes, logsRes, obsRes] = await Promise.all([
      ...
      supabase
        .from('reviews')
        .select(
          'id, rating, comment, visited_at, photo_urls, created_at, user:users!reviews_user_id_fkey(nickname)'
        )
        .eq('place_id', id)
      ...
      supabase
        .from('bottle_logs')
        .select(
          'id, free_label, context, price, currency, fx_to_krw, photo_url, logged_at, product:products(display_name), user:users!bottle_logs_user_id_fkey(nickname)'
        )
        .eq('place_id', id)
